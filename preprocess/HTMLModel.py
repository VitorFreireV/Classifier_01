...
class Pre_processing_HTML:
    ...
    def set_featureslist(self, features_list):
        config_list = {}
        for key in self.config_html.keys():
            if key in features_list:
                config_list[key] = self.config_html[key]
        self.config_html = config_list
    ...
    def find_numberHyperlinks2(self, page):
        domain = page['domain']
        soup = page['soup']
        count = 0
        count_all = 0
        ...
        for link in list_links:
            if validators.url(link):
                ...
                try:
                    if link[:4] != 'http':
                        link = 'http://' + link
                    req = requests.head(link, verify = True, timeout = 1)
    ...
    def get_Copyrightfeatures(self, page):
        ...
        if copyrightTexts:
            # Try pois copyrighTexts pode ser formado apenas por StopWords, quebrando o CountVectorizer()
            try:
    ...
    def get_dffromlist(self, list_html):
        bar = IncrementalBar('HTML Features Progress:', max = len(list_html))
